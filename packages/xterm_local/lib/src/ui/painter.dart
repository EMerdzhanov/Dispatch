...
class TerminalPainter {
  ...
  void paintCellForeground(Canvas canvas, Offset offset, CellData cellData) {
    ...
    if (paragraph == null) {
      ...
      // When drawBoldTextInBrightColors is enabled, shift standard
      // colors (0-7) to their bright variants (8-15) for bold text.
      if (isBold && _textStyle.drawBoldTextInBrightColors) {
        final colorType = cellData.foreground & CellColor.typeMask;
        final colorValue = cellData.foreground & CellColor.valueMask;
        if ((colorType == CellColor.named || colorType == CellColor.palette) && colorValue < 8) {
          color = _colorPalette[colorValue + 8];
        } else if (colorType == CellColor.normal) {
          // Default foreground in bold → use bright white
          color = _colorPalette[15];
        }
      }
